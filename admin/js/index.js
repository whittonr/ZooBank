...
function general_search(taxonid) {
	...
	var search_term = document.getElementById("taxon_act_search_string").value;
	if(taxonid==undefined) {
		$("#taxon_act_search_string").addClass("loading");
	...
	var get_acts = $.ajax({
		...
		success: function(msg){
			...
			if(taxonid==undefined) find_taxon_act_response_action(msg,search_term);				
			else get_taxon_details_response_action(msg,search_term);
			$("#taxon_act_search_string").removeClass("loading");
			}//end success
...
function get_gni_data_response_action(results,term,add_register_link){
	...
	if(results.name_strings==undefined) gni_details_html = "No GNI matches";
	else {
