...
function add_author_to_list(PKID,author_name,zblsid){
	...
	if($.inArray(PKID, selected_authors_array)<0){
		...
		else new_author_html = new_author_html + ' <span style="position:relative;left:320px;"><img src="\/images\/lsid-token.gif" \/><object width="110" height="14" id="clippy" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000"><param value="\/public\/flash\/clippy.swf" name="movie"><param value="always" name="allowScriptAccess"><param value="high" name="quality"><param value="noscale" name="scale"><param value="text='+zblsid+'" name="FlashVars"><param value="#f5f5ff" name="bgcolor"><embed width="110" height="14" bgcolor="#f5f5ff" flashvars="text='+zblsid+'" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" allowscriptaccess="always" quality="high" name="clippy" src="\/public\/flash\/clippy.swf"></object><\/span>';
...
function launch_find_reference(){
	...
	//alert($("#ParentReferenceID").val());
	find_reference(author_id_list,$("#pub_year").val(),$("#ParentReferenceID").val(),$("#pub_title").val());
}

function find_reference_response_action(results,author_id_list,publication_year,journal_id,reference_title){
	var pub_list_html = "<table>";
	pub_list_html = pub_list_html + '<tr><td colspan="2" style="width:800px;"><div class="infoMsg"><p>Review the list of Published Works below and select the Work involved with this Registration. If the Work is not on the list, ensure this is the correct person, and select (or create) the name of the author <em>exactly</em> as it appears in the Published Work itself.<br \/>'+results.length+' Publication';
	if(results.length>1) pub_list_html = pub_list_html + 's';
	pub_list_html = pub_list_html + '<\/p><\/div><br \/><\/td><\/tr>';
	
	
	if(results.length==0) pub_list_html = "<tr><td>No Results<\/td><\/tr>";
	else if(results.length>199) pub_list_html = "<tr><td>Many results<\/td><\/tr>";
	...
	for(x=0;x<results.length;x++){
		...
		full_citation = results[x].cheatauthors + ' ' + results[x].Year +'. '+results[x].Title+' '+results[x].CheatCitationDetails;
		pub_list_html = pub_list_html + '<tr class="'+row_class+'"><td>' + full_citation +'<\/td><td><button type="button" onclick="select_publication('+results[x].id+',&quot;'+full_citation+'&quot;,&quot;'+results[x].cheatauthors+'&quot;);$( &quot;#select_author&quot; ).dialog(&quot;close&quot;);$( &quot;#pub_layer&quot; ).dialog(&quot;close&quot;);">'+button_text+'<\/button><\/td><\/tr>';
