...
<cfcomponent>
	...
	<cffunction name="insert_author" hint="inserts a new author" access="remote" returntype="struct" returnformat="json">
		...
		<cfargument name="LinkID" type="numeric" required="no">
		...
		<cfelse><!--- do the insert --->
			<cfquery name="insert_person" datasource="#datasource#">
				...
				EXEC	sp_createPerson @GivenName=<cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.GivenName#">,  
				...
				<cfif Arguments.LinkID gt 0>
					@ValidAgentID=<cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.LinkID#">,
				</cfif>
				@PKID=@result output
				
				select @result as PKID
				
			</cfquery>
			<cfset result.message = "success">
			<cfset result.PKID = insert_person.PKID>
	...
	<cffunction name="create_LSID" hint="Creates a new registration for an item, and returns the LSID" access="public">
		...
		<cfquery name="register_name" datasource="#datasource#">
			...
			EXEC	sp_AssignZooBankLSID @PKID= <cfqueryparam cfsqltype="cf_sql_integer" value="#Arguments.PKID#">,
	...
	<cffunction name="insert_reference" hint="Inserts a new reference item" access="remote" returntype="struct" returnformat="json">
		<cfargument name="ParentReferenceID" type="string" required="no">
		...
		<cfargument name="Authors" type="string" required="yes" hint="A comma delimited list of Agent PKs">
		<cfargument name="LogUserName" type="string" required="yes" hint="The user name of the user entering the reference">
		<!---@ParentReferenceID	int				=0,
		...
		<cfinvoke method="find_pub" returnvariable="check_duplicate_pub">
			<cfinvokeargument name="term" value="#Arguments.Title#">
		</cfinvoke>
		<cfscript>
			duplicate_response = Deserializejson(check_duplicate_pub,true);
		</cfscript>
		<cfset function_result = StructNew()>
		<cfif duplicate_response[1].id is 0>
			<cfquery name="insert_reference" datasource="#datasource#">
				DECLARE @result int
				EXEC	sp_CreateReference 
				@ParentReferenceID =<cfif Arguments.ParentReferenceID gt 0><cfqueryparam cfsqltype="cf_sql_integer" value="#Arguments.ParentReferenceID#"><cfelse>0</cfif>,
				...
				@Year	= <cfif Arguments.Year gt 0><cfqueryparam cfsqltype="cf_sql_integer" value="#Arguments.Year#"><cfelse>0</cfif>,
				...
				@DatePublished	= <cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.DatePublished#">,
				@Authors	= <cfqueryparam cfsqltype="cf_sql_varchar" value="#Replace(Arguments.Authors,",","|","ALL")#|">,
				@LogUserName = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.LogUserName#">,
				@PKID=@result output
			...
			<cfinvoke method="find_pub" returnvariable="new_citation_value">
				<cfinvokeargument name="ReferenceID" value="#insert_reference.PKID#">
			</cfinvoke>			
			<cfscript>
				new_citation_response = Deserializejson(new_citation_value,true);
			</cfscript>
			<!---set the LSID--->
			<cfinvoke method="create_LSID" returnvariable="new_reference_LSID">
				<cfinvokeargument name="PKID" value="#insert_reference.PKID#">
				<cfinvokeargument name="DoRegister" value="1">
				<cfinvokeargument name="LogUserName" value="#Arguments.LogUserName#">			
			</cfinvoke>
			<!---<cfset function_result = '[{"pk":"#insert_reference.PK#","new_citation":"#Arguments.Title#"}]'>--->
			
			<cfset function_result.pk = insert_reference.PKID>
			<cfif Arguments.ReferenceTypeID is 39>
				<cfset function_result.new_citation = "<i>#new_citation_response[1].Title#</i>">
			<cfelse>
				<cfset function_result.new_citation = "#new_citation_response[1].CheatFullAuthors# #new_citation_response[1].Year# #new_citation_response[1].Title# #new_citation_response[1].CheatCitationDetails#">
			</cfif>
			<cfset function_result.message = "Success">
			<cfset function_result.lsid = new_reference_LSID>
			<cfset function_result.cheatauthors = new_citation_response[1].CheatAuthors>
		<cfelse>
			<cfset function_result.pk = duplicate_response[1].id>
			<cfset function_result.new_citation = arguments.title>
			<cfset function_result.message = "Publication already exists.">
		</cfif>
		
		<cfreturn function_result />
	</cffunction>
	...
	<cffunction name="insert_taxon_name" hint="Inserts a new name" access="remote" returnformat="JSON" returntype="struct">
		<cfargument name="ReferenceID" type="numeric" required="yes">
		<cfargument name="ProtonymID" type="string" required="no">
		<cfargument name="TaxonRankID" type="numeric" required="no">
		<cfargument name="ParentUsageID" type="string" required="no">
		...
		<cfquery name="insert_name" datasource="#datasource#">		
			DECLARE @result int
			EXEC	sp_CreateTaxonNameUsage
			@ReferenceID = <cfqueryparam cfsqltype="cf_sql_integer" value="#Arguments.ReferenceID#">,
			@TaxonRankID = <cfqueryparam cfsqltype="cf_sql_integer" value="#Arguments.TaxonRankID#">,
			<cfif Arguments.ParentUsageID gt 0>
				@ParentUsageID = <cfqueryparam cfsqltype="cf_sql_integer" value="#Arguments.ParentUsageID#">,
			</cfif>
			@NameString = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.NameString#">,
			@Pages = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.Pages#">,
			@LogUserName = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Arguments.LogUserName#">
			<cfif arguments.ProtonymID gt 0>, @ProtonymID = <cfqueryparam cfsqltype="cf_sql_integer" value="#Arguments.ProtonymID#"></cfif>
			,@PKID=@result output
			select @result as PKID			
		</cfquery>
		<cfset result.message = "Created Taxon Name Usage (PK:#insert_name.PKID#)">
